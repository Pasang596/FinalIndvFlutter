...
class SalonPendingPage extends StatefulWidget {
  const SalonPendingPage({Key? key}) : super(key: key);

  @override
  State<SalonPendingPage> createState() => _SalonPendingPageState();
}

class _SalonPendingPageState extends State<SalonPendingPage> {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                FutureBuilder<List<GetSalonUIAppointment?>>(
                  future: CategoryRepository().getSalonUIAppointment("Bidding"),
  ...
  Widget scheduleContainer(
      GetSalonUIAppointment? appointmentResponse, int index) {
    return Padding(
                      ...
                      if (appointmentResponse.status! == "Bidding")
                        SizedBox(
                          ...
                          child: ElevatedButton(
                            ...
                            onPressed: () async {
                              _updateSalonAppointmentStatus(
                                  appointmentResponse.id!, "Accepted");
                              Navigator.of(context).push(
                                MaterialPageRoute(
                                  builder: (context) =>
                                      const BottomNavSalon(index: 2),
                                ),
                              );
  ...
  _updateSalonAppointmentStatus(String id, String status) async {
    bool isUpdated =
        await CategoryRepository().updateSalonUIAppointment(id, status);
  }
  ...
  _displayMessage(bool isSignUp) {
    if (isSignUp) {
      ...
      Future.delayed(const Duration(milliseconds: 1500), () {
        ...
        Navigator.of(context).push(
          MaterialPageRoute(
              builder: (context) => const BottomNavSalon(index: 0)),
        );
      });
    } else {
  ...
  acceptAppointment(GetSalonUIAppointment appointmentResponse) async {
    debugPrint("Appointment ID: " + appointmentResponse.id!);
    bool isAccepted = await CategoryRepository()
        .updateSalonUIAppointment(appointmentResponse.id!, "Accepted");
    if (isAccepted) {
      ...
      Future.delayed(
        ...
        () {
          ...
          Navigator.of(context).push(
            MaterialPageRoute(
                builder: (context) => const BottomNavSalon(index: 1)),
          );
        },
      );
    } else {
  ...
  deleteAppointment(GetSalonUIAppointment appointmentResponse) async {
    bool isDeleted;
